...
The LLM (`medgemma1.5:4b-it-q4_K_M`) is the decision-maker. The pipeline MUST provide context and minimal
...
Ollama (`medgemma1.5:4b-it-q4_K_M`), and httpx for async HTTP. These constraints exist because the
...
- Ollama: 120s total timeout, 32K context window via `medgemma1.5:4b-it-q4_K_M`.
