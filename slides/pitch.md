...
**Stack:** FastAPI + Next.js 16 + Ollama (`qwen2.5:7b`) + TypeScript + Tailwind
...
- Drug names extracted heuristically from query text (capitalized words + 5-15 char lowercase), not matched against fixed lists.
- Deleted 5 modules over the project: prompts.py, query_classifier.py, classifier_data.py, pubmed_client.py, semantic_scholar_client.py — each replaced by simpler, generative alternatives.

**Evidence of AI usage:**
- **MCP:** `.claude/mcp.json` — GitHub MCP server for issue/PR management
- **Skill:** `.claude/skills/medical-rag/SKILL.md` — enforces FastAPI + Ollama + live APIs, forbids hardcoded prompts and classifiers
...
**3. Fully local and free.**
`qwen2.5:7b` runs on-device via Ollama. Wikipedia and OpenFDA need no API keys. Zero cost per query, zero data leaves your machine.

**4. Built the hard way — then simplified.**
