...
- what: Project-scoped skill enforcing architectural constraints — FastAPI backend, Next.js + React + Tailwind frontend, local Ollama (medgemma1.5:4b-it-q4_K_M), Wikipedia MediaWiki + OpenFDA Drug Label live APIs, Supabase for auth + persistence. Forbids hardcoded system prompts, keyword classifiers, drug lists, Flask, Django, WebSockets, ChromaDB, FAISS, cloud LLMs, and any local vector store. Enforces SSE wire format (6 event types: token, citation, done, error, warning, info), `[[CITATION:N]]` markers rendered as clickable `[Wikipedia ↗]` (teal) / `[FDA ↗]` (amber) inline tags, concurrent API calls via `asyncio.gather()`, and Supabase JWT auth on all session routes. Pipeline: any query → Wikipedia (raw search) + drug extraction (heuristic) → OpenFDA (concurrent) → minimal prompt ("answer helpfully, cite sources, include disclaimer") → medgemma1.5:4b-it-q4_K_M streaming response.
